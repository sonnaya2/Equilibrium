...
4. Trace the complete path before changing it:
   ...
   - runtime: `src/combat/engine/runtime/`, resolution accounting, sampled poison state, lane aggregation
   - output: per-ability totals, analysis sources, events, timeline, support labels
5. Reuse the queue, immutable `RotationState`, provenance capabilities, target-owned poison state, and existing loadout normalization. Do not build a second poison simulator.
6. Add the smallest focused tests that prove formula, cadence, refresh, continuation, deterministic repeats, serialization, score/full parity, and attribution.
...
- Store one concrete poison sample per stochastic lane: effective tier, expiration, remaining hits, cadence, decay index, next hit, earned delayed-hit carriers, source multiplier, and immunity clock.
- Application and Cinderbane continuation change future target state, so sample them through the canonical 128-lane stratified oracle. Never enumerate poison futures, intern poison distributions, cap live states, retry with more lanes, or discard probability mass.
- Keep the poison damage range as expected damage with minimum and maximum analysis bounds.
- Require a finite production horizon for recursive Cinderbane chains. Fixed-window runs must retain unit mass and report zero residual mass.
...
- Every Cinderbane poison hit makes the same single continuation roll. A success refreshes poison and earns another +2-tick hit. Preserve this state transition; use `p / (1 - p)` only as an aggregate test oracle.
...
- Show poison source, effective tier, poison proc chance, current minimum/expected/maximum damage, applications, separate hits, decay index, current Evolving Toxin stacks, remaining poison duration, and support status from aggregate engine output. `summary.events` is one representative sampled history and must not be summed for expected counts. Do not calculate poison in React.
...
```text
npx vitest run src/combat/engine/simulation/playerPoison.test.ts src/combat/engine/runtime/stochastic.test.ts src/combat/engine/simulation/summary.test.ts
npm run test:combat
...
- 12.5% and Laniakea 17.5% application rates within fixed-lane tolerance
...
- 60-100 tick Avernic plus Cinderbane plus poison plus adrenaline perks
- probability mass 1, residual mass 0, bounded memory, deterministic repeats, and score/full parity
- total damage within 1% of the retired exact oracle for the combined workload
