...
- `data/seed-v1.json.gz` is the retired baseline. Nothing builds from it except the temporary comparison commands below; do not edit or open it.
...
The importer is `scripts/data/canonical/import.mjs`. It reads declared fields only — no type inferred from a filename, no ID derived from a name, no key-path search, no alias spellings, no keyword classification — and runs in one transaction with foreign keys on, so a bad record names its file, line, record and reason and leaves nothing behind. Keep it that way; the legacy inference lives in `ingest.mjs` and `normalize.mjs` and is on its way out.

Until Stage 3 retires it, the pre-canonical build survives as `npm run data:rebuild:legacy-seed`, and `npm run data:parity:legacy-canonical` proves the two paths produce the same database and byte-identical artifacts. Run the parity command after any change to ingestion, the canonical schema, or export.
