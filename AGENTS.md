...
tracked — an explicit JSONL mirror of the validated database, reviewable in a diff, never
hand-edited, regenerated by `data:canonical:export` and checked by `data:canonical:validate`
(`docs/canonical-data.md`). Do not add a hosted database, API, CMS, or second authoring tree.
Architecture and commands: `docs/data-platform.md`; the legacy audit behind it:
`docs/legacy-data-stage0.md`.
