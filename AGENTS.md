...
**One shipped data store.** Root `data/` is canonical. `scraped-data/` is an ignored local research
input, and `public/data/v1/research/` is generated by `npm run build:data`; neither is a second
authoring store. Do not add a database, API, CMS, or parallel copy under `src/`.
