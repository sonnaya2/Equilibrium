...
**One editable data system.** Root `data/` contains the canonical JSONL dataset, SQL migrations, and
small JSONL content patches — that is the whole of the tracked authoring surface.
`.cache/equilibrium.sqlite`, `public/data/v2/`, and the data reports are generated
build artifacts and are never committed. `data/canonical/` is the one generated tree that *is*
tracked, and since Stage 2 it is also the build input — an explicit JSONL mirror of the validated
database, reviewable in a diff, never hand-edited, regenerated by `data:canonical:export` and checked
by `data:canonical:validate` (`docs/canonical-data.md`). `data/seed-v1.json.gz` is retired: it feeds
only `data:rebuild:legacy-seed` and `data:parity:legacy-canonical`, both temporary. Do not add a
hosted database, API, CMS, or second authoring tree. Architecture and commands:
`docs/data-platform.md`; the legacy audit behind it: `docs/legacy-data-stage0.md`.

### Data editing rules

1. Never open or rewrite a complete generated dataset to change one record.
2. Never edit `.cache/equilibrium.sqlite`, `public/data/`, or `data/canonical/` manually.
...
6. Re-export `data/canonical/` and commit it with the patch that changed it.
7. Do not build from the retired seed outside the two temporary comparison commands.
...
# Temporary, deleted in Stage 3:
npm run data:rebuild:legacy-seed      # the pre-canonical build, for comparison only
npm run data:parity:legacy-canonical  # builds both and compares rows and artifacts
```
