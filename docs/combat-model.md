...
- Style damage bonus enters **inside** the weapon-term floor: `1` main-hand; `1.5` for two-handed weapon term; off-hand path floors a half main-hand term.
...
- Two-handed (all styles): `floor(DPL) + floor(DPL/2)` level terms + single weapon floor `floor(14.4 × min(tier, styleCap) + 1.5 × styleBonus)`.
