...
describe("playerVitality", () => {
  it("heals up to max and records overheal", () => {
    const r = applyPlayerHeal(
      { currentLifePoints: 8_000, maximumLifePoints: 10_000 },
      10_000,
    );
  ...
  it("applies full heal when room exists", () => {
    const r = applyPlayerHeal(
      { currentLifePoints: 1_000, maximumLifePoints: 20_000 },
      10_000,
    );
  ...
  it("clamps current when max drops", () => {
    ...
    expect(
      clampCurrentLife({ currentLifePoints: 5_000, maximumLifePoints: 4_000 }),
    ).toEqual({ currentLifePoints: 4_000, maximumLifePoints: 4_000 });
  });

  it("marks wouldDie when life reaches 0", () => {
    const r = applyIncomingPlayerDamage(
      { currentLifePoints: 500, maximumLifePoints: 10_000 },
      500,
    );
