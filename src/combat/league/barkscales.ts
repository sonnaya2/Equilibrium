...
export interface BarkscalesOutcome extends BarkscalesReduction {
  ...
  /** Qualifying incoming hits over the window, or null with no scenario. */
  qualifyingHits: number | null;
  /** Grasp triggers over the window, or null with no scenario. */
  triggers: number | null;
  /** Reductions banked toward the next trigger at the end of the window. */
  counterRemainder: number | null;
  /** Seconds between triggers, or null with no scenario. */
  secondsPerTrigger: number | null;
  /** Targets each Grasp damages; poison immunity zeroes it. */
  targetsStruck: number;
  /** Total mitigated damage over the window, or null with no scenario. */
...
export function barkscalesOutcome(
  ...
  const targetsStruck = scenario.poisonImmune
    ? 0
    : Math.max(1, Math.floor(scenario.targetsStruck ?? 1));
  ...
  if (interval === undefined || !Number.isFinite(interval) || interval <= 0) {
    return {
      ...
      missingInputs: ["Incoming qualifying-hit interval"],
      perHit,
      hitsPerTrigger,
      qualifyingHits: null,
      triggers: null,
  ...
  const qualifyingHits = Math.floor(Math.max(0, windowSeconds) / interval);
  return {
    support: "modeled",
    missingInputs: [],
    perHit,
