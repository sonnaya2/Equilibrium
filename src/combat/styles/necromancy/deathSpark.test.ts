...
describe("Death Spark pure helpers", () => {
  it("stacks 0→1→2→3→4 then empower resets to 0", () => {
    expect(DEATH_SPARK_STACKS_TO_EMPOWER).toBe(4);
...
describe("Death Spark sim: Omni Guard stacks and empowered basic", () => {
  ...
  it("builds stacks on basic; 5th basic is double damage and resets stacks", () => {
    ...
    // 5th basic: empower (double band) then stacks → 0
    ...
    expect(castBasics.length).toBeGreaterThanOrEqual(5);

    // Fixture AD 1000, band 90-110 → expected 1000; empowered → 2000.
    for (const c of castBasics.slice(0, 4)) {
      expect(c.result.hits[0]!.expected).toBeCloseTo(1000, 5);
    }
    expect(castBasics[4]!.result.hits[0]!.expected).toBeCloseTo(2000, 5);
  });
