 ...
 * fixed fractions of the cumulative stored amount; stacks are not consumed.
...
export const PUNCTURE_HIT_FRACTIONS = [0.5, 0.2, 0.15, 0.1, 0.05] as const;
...
export function activePuncture(state: PunctureState, tick: number): PunctureState {
  return tick < state.expiresAtTick
    ? state
    : { stacks: 0, expiresAtTick: 0, storedDamage: 0, generation: state.generation, pendingOwnerCast: -1 };
}
...
export function punctureStoreAmount(abilityDamage: number): number {
  ...
  return Math.floor((abilityDamage * PUNCTURE_STORE_PCT) / 100);
}
...
/** Fixed damage for one sequence hit (snapshotted stored amount × fraction). */
export function punctureHitDamage(storedDamage: number, fraction: number): number {
  if (storedDamage <= 0 || fraction <= 0) return 0;
  return Math.floor(storedDamage * fraction);
}
...
export function punctureSequenceTicks(firstTick: number): readonly number[] {
  return PUNCTURE_HIT_FRACTIONS.map(
