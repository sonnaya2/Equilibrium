 ...
 * Keep row-span focus band. Full filtered set via row virt (no 120 cap).
 * Closed tiles denser + Cipher mono Comp%/pts ribbon. Idle chrome dead.
...
const CREST = 22;
/** Must match --sg-min-card (13.5rem) and --sg-gap (0.35rem) in sigil.css */
const MIN_CARD_PX = 13.5 * 16;
const GAP_PX = 0.35 * 16;
/** Closed row: tile height + row gap. */
const ROW_EST_PX = 118;
/** Focus band under open row (measureElement corrects). */
const ROW_BAND_EXTRA_PX = 168;

function useGridColumns(
  listRef: RefObject<HTMLDivElement | null>,
  /** Re-bind when board mounts (empty → filtered) */
  boardKey: number,
): number {
...
function Tile({
  ...
  const domId = `sigil-r2-${id.replace(/[^a-zA-Z0-9_-]+/g, "-")}-${index}`;
  ...
  return (
    ...
    >
      ...
      {/* Cipher-grade mono ribbon — scan line; region lives here once */}
      <div
      ...
      {/* Foot: cue only — no region restatement (Ash rule) */}
