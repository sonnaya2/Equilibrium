...
/**
 * Cipher Strip R3 DEATH FINALS — ribbon + full virt + Sigil band + denser cols + Quill track.
 * Steals Quill shell/track height + crest rail; Grove column pressure; kills closed cue.
 * Signature stays mono Comp%/pts ribbon. Concepts lab only.
 */
...
/** ~12rem — denser than R2 12.75; still holds 15px names + mono ribbon. */
const MIN_CARD_PX = 12 * 16;
const GAP_PX = 0.3 * 16;
/** Closed row without cue foot: head + ribbon + gap. */
const ROW_EST_PX = 96;
/** Focus band under selected row (measureElement corrects live). */
  ...
  return (
    <div className="td-gw-cipher">
      ...
      <div className="td-gw-cipher__shell">
        {/* Quill-thin track: fixed cluster + nested crest rail — filters never hide */}
        <div className="td-gw-cipher__track" role="toolbar" aria-label="Task filters">
          <div className="td-gw-cipher__cluster">
            ...
            {startersOnly ? (
              <span className="td-gw-cipher__note">
          ...
          {regionRail.length > 0 ? (
            <>
              <span className="td-gw-cipher__rule" aria-hidden />
              <div
                role="group"
                aria-label="Filter by region"
                className="td-gw-cipher__regions"
              >
                <button
                  type="button"
                  className={`td-gw-cipher__crest-btn${region === "all" ? " is-on" : ""}`}
                ...
                >
                  <span className="td-gw-cipher__crest-glyph" aria-hidden>
                    Σ
                  </span>
                  <span className="td-gw-cipher__crest-n">
                ...
                {regionRail.map((id) => {
                  ...
                  return (
                    <button
                      key={id}
                      type="button"
                      className={`td-gw-cipher__crest-btn${
                        region === id ? " is-on" : ""
                      }${locked ? " is-locked" : ""}`}
                    ...
                    >
                      ...
                      <span className="td-gw-cipher__crest-n">{count}</span>
                    </button>
                  );
                })}
              </div>
...
function CipherTile({
  ...
  const domId = `cipher-r3-${id.replace(/[^a-zA-Z0-9_-]+/g, "-")}`;
  ...
  return (
    ...
    >
      ...
      {/* No cue foot — open state is gem border + band; cue was residual height tax */}
    </article>
  );
}
